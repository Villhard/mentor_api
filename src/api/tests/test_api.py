...
class TestUrls(APITestCase):
    ...
    def setUp(self):
        ...
        self.test_user_client.credentials(
    ...
    def test_refresh_token_success(self):
        ...
        response = self.test_user_client.post(
            self.urls["refresh"], payload, format="json"
        )
    ...
    def test_logout_success(self):
        ...
        response = self.test_user_client.post(
            self.urls["logout"], payload, format="json"
        )
    ...
    def test_user_list_success(self):
        ...
        self.assertIn(
            "test_user", [user["username"] for user in response.data["results"]]
        )
        self.assertIn(
            "has_mentees", [user["username"] for user in response.data["results"]]
        )
        self.assertIn(
            "has_mentor", [user["username"] for user in response.data["results"]]
        )
        self.assertIn(
            "has_both", [user["username"] for user in response.data["results"]]
        )
        self.assertIn(
            "mentee1", [user["username"] for user in response.data["results"]]
        )
        self.assertIn(
            "mentee2", [user["username"] for user in response.data["results"]]
        )
        self.assertEqual(
            len([user for user in response.data["results"] if user["is_mentor"]]),
            mentor_count,
        )
    ...
    def test_user_with_mentees_detail_success(self):
        ...
        mentees_usernames = [
            mentee.username for mentee in self.user_with_mentees.mentees.all()
        ]
    ...
    def test_user_with_both_detail_success(self):
        ...
        mentees_usernames = [
            mentee.username for mentee in self.user_with_both.mentees.all()
        ]
    ...
    def test_user_update_success(self):
        ...
        response = self.test_user_client.put(
            self.urls["user_detail"](user_id), payload, format="json"
        )
    ...
    def test_registration_with_invalid_data(self):
        ...
        for payload in payloads:
            response = self.client.post(
                self.urls["registration"], payload, format="json"
            )
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_update_other_user_forbidden(self):
        ...
        response = self.test_user_client.put(
            self.urls["user_detail"](other_user.id), payload, format="json"
        )
    ...
    def test_set_nonexistent_mentor(self):
        payload = {"mentor": "nonexistent_user"}

        response = self.test_user_client.put(
            self.urls["user_detail"](self.test_user.id), payload, format="json"
        )
